...
public class NewsTblsGenerator extends DbRelatedWorkerBase {

	private final String root = ".\\src\\main\\resources\\";
	private String[] providers;
	private int[] years;

	public NewsTblsGenerator(String dbConnString, String dbUsername, String dbPassword, int[] years,
			String[] providers) {
	...
	public WorkReport execute(WorkContext ctx) {
		System.out.println("EXECUTING CREATE NEWS TABLES");
		try {
			for (String provider : this.providers) {
				String sqlFile = root + "news_" + provider.toLowerCase() + ".sql";
				File sql = new File(sqlFile);
				if (sql.isFile()) {
					for (int year : this.years) {
						String raw = new String(Files.readAllBytes(sql.toPath()));
						String content = raw.replaceAll("\\$1", year + "");
	...
	public static void main(String[] args) {
		NewsTblsGenerator worker = new NewsTblsGenerator(Config.CONN_STRING, Config.DB_USERNAME, Config.DB_PASSWORD,
				Config.YEARS, Config.NEWS_PROVIDERS);
